...
export default function Footer() {
  return (
    <footer className="bg-black text-white py-14 snap-start min-h-[30vh] flex items-center border-t border-white/10">
      <div className="max-w-5xl mx-auto px-6 w-full flex flex-col items-center gap-8">
        <div className="flex gap-4">
          {socialLinks.map((item) => (
            <a
              ...
              className="w-12 h-12 rounded-full bg-white text-black flex items-center justify-center hover:bg-[#C3FF1F] transition-colors shadow-lg shadow-black/40"
            >
              {item.icon}
            </a>
          ))}
        ...
        <p className="text-xs tracking-[0.35em] uppercase text-white/60 text-center">
          © 2024 Kraft Studio — Designed by Nova Hub
        </p>
      </div>
    </footer>
  )
